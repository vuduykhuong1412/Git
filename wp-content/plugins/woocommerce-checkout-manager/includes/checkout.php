		...
		foreach( $buttons as $btn ) {
			if( $btn['type'] == 'wooccmtextarea' ) {
				if( !empty( $_POST[ 'shipping_' . $btn['cow'] ] ) ) {
					update_post_meta( $order, '_shipping_' . $btn['cow'] , wp_kses( $_POST['shipping_' . $btn['cow']], false ) );
				}
			}
		}
	}
		...
		foreach( $buttons as $btn ) {
			if( $btn['type'] == 'wooccmtextarea' ) {
				if( !empty( $_POST[ 'billing_' . $btn['cow'] ] ) ) {
					update_post_meta( $order, '_billing_' . $btn['cow'] , wp_kses( $_POST['billing_' . $btn['cow']], false ) );
				}
			}
		}
	}
...
function wooccm_custom_checkout_field_update_user_meta( $user_id = 0, $posted ) {
	...
	if( !empty( $buttons ) ) {
		foreach( $buttons as $btn ) {
			if( $btn['type'] == 'wooccmtextarea' ) {
				if( !empty( $_POST['shipping_' . $btn['cow']] ) ) {
					update_user_meta( $user_id, 'shipping_' . $btn['cow'] , wp_kses( $_POST['shipping_' . $btn['cow']], false ) );
				}
			}
		}
	}
	...
	if( !empty( $buttons ) ) {
		foreach( $buttons as $btn ) {
			if( $btn['type'] == 'wooccmtextarea' ) {
				if( !empty( $_POST['billing_' . $btn['cow']] ) ) {
					update_user_meta( $user_id, 'billing_' . $btn['cow'] , wp_kses( $_POST['billing_' . $btn['cow']], false ) );
				}
			}
		}
	}
