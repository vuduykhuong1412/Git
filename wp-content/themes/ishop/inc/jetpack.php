...
function ishop_jetpack_setup() {
	...
	add_theme_support( 'infinite-scroll', array(
		'container' => 'main',
		'render'    => 'ishop_infinite_scroll_render',
		'footer'    => 'page',
		'wrapper'	=> false,
...
function ishop_infinite_scroll_render() {
	echo '<ul class="medium-block-grid-3 large-block-grid-3 latest-post"><li>';
	while ( have_posts() ) {
		the_post();
		if ( is_search() ) :
...
function ishop_theme_jetpack_infinite_scroll_supported() {
	return current_theme_supports( 'infinite-scroll' ) && ( is_home() || is_archive() || is_search() ) && ! is_post_type_archive( 'product' );
}
add_filter( 'infinite_scroll_archive_supported', 'ishop_theme_jetpack_infinite_scroll_supported' );